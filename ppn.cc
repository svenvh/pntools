 ...
 *
 *     $Id: ppn.cc,v 1.16 2012/01/18 15:38:22 tzhai Exp $
 */
...
PPN_ADG::PPN_ADG(adg *pn_adg, isl_ctx *ctx){
	this->pn_adg= pn_adg;
	this->ctx = ctx;
}
...
// load a XXX-pn.yaml file
void PPN_ADG::Load(FILE *fp, isl_ctx *ctx)
{

	this->pn_adg = adg_parse(ctx, fp);
	assert(pn_adg);
...
Channels
PPN_ADG::getChannels(){
		return this->pn_adg->edges;
}

Channels
PPN_ADG::getChannels(const Process *process){
	Channels channels_proc;

	Channels channels = this->pn_adg->edges;
			...
			++cit)
	{
		...
		if (strcmp(isl_id_get_name(ch->from_node_name), isl_id_get_name(ch->to_node_name)) != 0) {
			continue;
		} else {
			channels_proc.push_back(ch);
		}
	}

	return channels_proc;
}


Processes
PPN_ADG::GetProcesses(){
	return this->pn_adg->nodes;
}

Domain*
PPN_ADG::getProcessDomain(Process *process){
	return process->domain;
}

__isl_give isl_set*
PPN_ADG::getProcessDomainBound(const Process *process){
...
Ports
PPN_ADG::getInPorts(const Process *process){
	return process->input_ports;
}


Ports
PPN_ADG::getOutPorts(const Process *process){
	return process->output_ports;
}


__isl_give isl_set*
PPN_ADG::getPortDomainBound(const Port *port){
...
PPN_ADG::GetParameters(){
	return this->pn_adg->params;
}
